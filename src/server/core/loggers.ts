...
export class Logger {
    ...
    constructor(prefix: string, logFileName?: string) {
        ...
        if (logFileName) {
            ...
            const fileLocation =
                projectRootPath + "/logs/server/" + logFileName + ".log";

            console.log(this._prefix, `Logging up in file: "${fileLocation}"`);
            this._destination = fs.createWriteStream(fileLocation, {
                flags: "w"
            });
    ...
    log(...messages: any[]): void {
        const message = `${this._prefix} ${messages
    ...
    private listenForDestinationErrors(): void {
        this._destination.on("error", err => {
            console.error(
                this._prefix,
                "An error occurred with the write stream. Check if the file exists.",
                err
            );
        });
    }
